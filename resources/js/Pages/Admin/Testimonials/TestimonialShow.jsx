...
export default function TestimonialShow() {
    ...
    const handleDelete = () => {
        if (testimonialToDelete) {
            destroy(route('testimonials.destroy', testimonialToDelete.id), {
                onSuccess: () => {
                    setShowModal(false);
                    ...
                    <table className="w-full table-auto text-sm text-left text-light-2">
                        <thead>
                            <tr className="bg-gradient-to-r from-dark-1/60 via-dark-1/40 to-dark-1/60 border-b border-primary-3/30">
                                ...
                                <th className="px-6 py-4 text-light-2 font-bold">Name</th>
                                <th className="px-6 py-4 text-light-2 font-bold">Position</th>
                                <th className="px-6 py-4 text-light-2 font-bold">Context</th>
                                <th className="px-6 py-4 text-light-2 font-bold">Ratings</th>
                            ...
                            {testimonials.map((testimonial) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">{testimonial.position}</td>
                                        <td className="px-6 py-4 max-w-xs truncate">{testimonial.context}</td>
                                        <td className="px-6 py-4">{testimonial.ratings}</td>
